...
def is_used_square(square):
    ...
    return is_taken(co_ords)



def is_taken(co_ords):
    row = co_ords[0]
    col = co_ords[1]
    if field[row][col] !="*":
        return True
...
def add_to_field(co_ords, active_user):
    row = co_ords[0]
    col = co_ords[1]
...
def victory(user):
    if check_row(user): return True
    if check_column(user): return True
    if check_diag(user): return True
...
while turns < 9:
    ...
    if is_taken(co_ords):
        ...
        continue
    add_to_field(co_ords, active_user)
    if victory(active_user):
        print_field()
